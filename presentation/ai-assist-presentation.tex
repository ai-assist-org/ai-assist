...
search_dci_jobs(query="...",
  __collect_to_report="jobs:jsonl:50")
\end{lstlisting}
...
\begin{itemize}\small
  ...
  \item \alert{Systemd user service}: \texttt{/service install} for persistent background monitoring
\end{itemize}
\end{column}
\begin{column}{0.50\textwidth}
\begin{lstlisting}[style=json,title={\scriptsize event-schedules.json}]
{
  "version": "2.0",
  "actions": [
    {
...
\begin{itemize}\small
  ...
  \item Deterministic workflow structure with dynamic reasoning
...
\textbf{Directives}
\begin{itemize}\small
  \item \texttt{@task} -- agent objectives with goals and exposed variables
  \item \texttt{@set} -- variable assignment
  \item \texttt{@if/@else} -- conditional execution
  \item \texttt{@loop} -- iterate over collections
  \item \texttt{@while} -- conditional loop (poll until done)
  \item \texttt{@wait} -- pause execution
  \item \texttt{@notify} -- emit user message
  \item \texttt{@return} -- workflow result
\end{itemize}
\end{column}
\begin{column}{0.48\textwidth}
...
\end{lstlisting}
\vspace{-2mm}
{\scriptsize\texttt{uv run ai-assist /run workflow.awl}}\\
{\scriptsize\textit{or ask the agent: ``run workflow.awl with project=CILAB''}}
\end{column}
\end{columns}
\end{frame}
...
\textbf{Agents can now execute AWL scripts dynamically}
...
\textbf{How it works}
\begin{itemize}\small
  \item New introspection tool: \texttt{execute\_awl\_script}
  \item Agent calls it when user references a \texttt{.awl} file
  \item Variables injected at runtime (no hardcoding)
  \item Results returned to conversation
\end{itemize}

\medskip
\textbf{Example prompt}\\
{\small\textit{``Run analysis.awl with project=CILAB and days=7''}}
\end{column}
\begin{column}{0.48\textwidth}
\begin{lstlisting}[style=code,language={}]
...
\begin{itemize}\small
  ...
  \item \alert{Success criteria} evaluated by Claude each cycle
...
\textbf{Features}
\begin{itemize}\small
  \item \texttt{/awl-viz} command opens flowchart in browser
  \item Dagre + D3.js for automatic graph layout
  \item Task blocks connected by arrows
  \item \alert{Variable names} shown on edges (data flow)
  \item If/else as diamond decision nodes with branches
  \item Loop/while as dashed-border clusters with back-arrows
  \item Mouse-over tooltips with full task details
  \item Zoom and pan support
\end{itemize}
\end{column}
\begin{column}{0.48\textwidth}
\textbf{Usage}
...
\begin{itemize}\small
  ...
  \item \textcolor{green!60!black}{\checkmark} Extended context (1M tokens on demand) + adaptive limits
  \item \textcolor{green!60!black}{\checkmark} Multi-instance support
  \item \textcolor{green!60!black}{\checkmark} AWL workflow execution from prompts
  \item \textcolor{green!60!black}{\checkmark} AWL workflow visualization (\texttt{/awl-viz})
  ...
  \item \textcolor{green!60!black}{\checkmark} \alert{Sandbox instances} (podman-compose, SSE transport, per-service credential isolation)
\end{itemize}
\end{column}
\begin{column}{0.48\textwidth}
\textbf{Future}
\begin{itemize}\small
  ...
  \item OpenShell policy layer (L7 network rules, dynamic credentials)
