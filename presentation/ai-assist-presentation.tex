...
\begin{itemize}
  ...
  \item \alert{Auto-context injection}: relevant KG entities injected into each query's system prompt via \textbf{semantic search} (vector embeddings)
\end{itemize}

\vfill
\centering
\small\color{gray} Solution: a \textbf{bi-temporal knowledge graph} stored in SQLite + \textbf{sqlite-vec} for vector search
\end{frame}
...
\begin{tikzpicture}[
  ...
  \node[data=rhred] (learnings) at (2.8,2.8)
    {6.\ \textbf{Relevant Learnings}\\(semantic search, top 5)};
  \node[data=rhred] (context) at (2.8,1.8)
    {7.\ \textbf{Auto-Context Entities}\\(semantic search, top 5)};

  % KG bracket
  \draw[decorate, decoration={brace, amplitude=4pt, mirror},
    thick, rhred!70!black] (5.1,1.45) -- (5.1,4.15)
    node[midway, right=5pt, font=\tiny\sffamily, text=rhred!70!black, align=left]
    {from KG via\\vector embeddings\\(\texttt{sqlite-vec})};

  % Arrow from query to prompt
  \draw[arr=rhblue] (query) -- (prompt)
    node[midway, right=3pt, font=\tiny\sffamily\color{gray}] {embed query};
...
\smallskip
\small\url{https://github.com/fredericlepied/ai-assist}
\end{frame}
