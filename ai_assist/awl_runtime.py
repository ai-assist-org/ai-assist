...
AWL_MODEL_LEVELS: tuple[str, ...] = ("low", "medium", "high", "default")
...
def _resolve_model_level(level: str | None, tiers: dict[str, str], default_model: str) -> str:
    ...
    A task with no level (``None``) resolves to the ``default`` tier, i.e. the
    session model (``default_model``). A level not present in ``tiers`` (which
    only holds low/medium/high overrides) also falls back to ``default_model``.
    """
    return tiers.get(level or "default", default_model)
